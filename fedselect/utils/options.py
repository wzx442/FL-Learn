...
def lth_args_parser():
    ...
    parser.add_argument("--lr", default=0.05, type=float, help="Learning rate 学习率")
    parser.add_argument("--batch_size", default=60, type=int, help="Batch size 批量大小")
    parser.add_argument("--lth_epoch_iters", default=3, type=int, help="LTH epoch iterations 本地交替优化迭代次数")
    parser.add_argument("--dataset", default="cifar10", type=str,help="dataset name 数据集名称")
    parser.add_argument("--arch_type",default="resnet18",type=str,help="architecture type 架构类型")
    parser.add_argument( "--setting", default="", type=str, help="setting name 设置名称" )
    parser.add_argument("--prune_percent", default=25, type=float, help="Pruning percent 剪枝百分比")
    parser.add_argument("--prune_target", default=80, type=int, help="Pruning target 剪枝目标")
    parser.add_argument( "--com_rounds", type=int, default=4, help="rounds of fedavg training 联邦平均训练轮数")
    parser.add_argument("--la_epochs", type=int,default=15,help="rounds of training for local alt optimization 本地交替优化训练轮数")
