    ...
    def train(self, net):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                # 在最后一个epoch的最后一个batch保存梯度
                if iter == self.args.local_ep - 1 and batch_idx == len(self.ldr_train) - 1:
                    for name, param in net.named_parameters():
                        if param.requires_grad and param.grad is not None:
                            final_gradients[name] = param.grad.clone().detach()
        ...
        # 返回模型的状态字典、所有训练周期的平均损失和最终梯度
        return net.state_dict(), sum(epoch_loss) / len(epoch_loss), final_gradients
